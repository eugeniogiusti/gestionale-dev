    ...
    <div x-show="open"
        ...
        <div class="mx-auto max-w-xl">
            <div class="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl" @click.stop>
                ...
                <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between">
                        <h3 class="text-base font-semibold text-gray-900 dark:text-white">
                            <span x-show="!isEdit">{{ __('meetings.create_meeting') }}</span>
                            <span x-show="isEdit">{{ __('meetings.edit_meeting') }}</span>
                ...
                <form method="POST"
                      :action="isEdit
                          ? '{{ route('meetings.update', [$project, '__MEETING_ID__']) }}'.replace('__MEETING_ID__', meetingId)
                          : '{{ route('meetings.store', $project) }}'">
                    @csrf
                    <input type="hidden" name="_method" x-bind:value="isEdit ? 'PUT' : 'POST'">

                    {{-- Form Fields --}}
                    <div class="px-4 py-3 max-h-[60vh] overflow-y-auto">
                        @include('meetings.partials._form-fields')
                    </div>
                    ...
                    <div class="px-4 py-3 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700 flex justify-end gap-2">
                        ...
                        <button type="submit"
                                :disabled="!formData.title || !formData.scheduled_at"
                                :class="(!formData.title || !formData.scheduled_at) ? 'bg-gray-300 dark:bg-gray-600 cursor-not-allowed' : 'bg-emerald-600 hover:bg-emerald-700'"
