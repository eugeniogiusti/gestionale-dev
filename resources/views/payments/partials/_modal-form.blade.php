    ...
    <div x-show="open"
        ...
        <div class="mx-auto max-w-xl">
            <div class="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl" @click.stop>
                ...
                <div class="px-4 py-3 border-b border-gray-200 dark:border-gray-700">
                    <div class="flex items-center justify-between">
                        <h3 class="text-base font-semibold text-gray-900 dark:text-white">
                            <span x-show="!isEdit">{{ __('payments.create_payment') }}</span>
                            <span x-show="isEdit">{{ __('payments.edit_payment') }}</span>
                        </h3>
                ...
                <form method="POST"
                      :action="isEdit
                          ? '{{ route('payments.update', [$project, '__PAYMENT_ID__']) }}'.replace('__PAYMENT_ID__', paymentId)
                          : '{{ route('payments.store', $project) }}'">
                    @csrf
                    <input type="hidden" name="_method" x-bind:value="isEdit ? 'PUT' : 'POST'">

                    {{-- Form Fields --}}
                    <div class="px-4 py-3 max-h-[60vh] overflow-y-auto">
                        @include('payments.partials._form-fields')
                    </div>
